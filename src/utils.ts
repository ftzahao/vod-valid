...
export const checkUrlConnectivity = async (url: string) => {
  ...
  const res = await fetch(url, { method: 'HEAD', signal: AbortSignal.timeout(5000) }).catch((error) => {
...
// 测试URL的连通性和延迟
export const testingUrlConnectivityAndLatency = async (url: string, { testTotal = 3, parallel = false, name = '' }) => {
  const totalTest = Math.max(1, testTotal) // 确保测试次数至少为1

  const results = []
  if (parallel) {
    let res = (
      await Promise.allSettled(
        Array.from({ length: totalTest }, async () => {
    ...
    for (let i = 0; i < totalTest; i++) {
      const start = performance.now()
      console.log(`第${i + 1}次测试开始: ${name} - ${url}`)
      const result = await checkUrlConnectivity(url)
      const end = performance.now()
      const latency = (end - start).toFixed(2)
      results.push({ result, latency })
      console.log(`第${i + 1}次测试完毕: ${name} - ${url}`)
    }
  }
  const successfulTests = results.filter((r) => r.result).length // 成功的测试次数
  const averageLatency = (results.reduce((sum, r) => sum + parseFloat(r.latency), 0) / totalTest).toFixed(2) // 平均延迟
...
export async function generateVodstream(sourceCsv: string) {
  const file = Bun.file(new URL('./vodstream_template.txt', import.meta.url).pathname)
  let content = await file.text()
  content = content.replaceAll('{RESOURCE_SITES}', sourceCsv).replaceAll('{version}', PackageJSON.version)
  return content
}
